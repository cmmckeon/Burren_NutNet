...
```{r table chunk, fig.height=20, fig.width=7.5}
...
get which species occurrence in which plots
```{r}
# 
# quick <- taxa_cover[taxa_cover$year == 2020 & taxa_cover$trt %in%  c("Control", "Fence", "NPK", "NPK+Fence"),]
# 
# change <- as.data.frame(unique(taxa_cover$taxa))
# for (i in  c("Control", "Fence", "NPK", "NPK+Fence")){
#   change[,i] <- 0
# }
# 
# names(change) <- c("taxa", "Control", "Fence", "NPK", "NPK+Fence")
#  
# for(i in levels(change$taxa)){
#   for(j in c("Control", "Fence", "NPK", "NPK+Fence")){
#       change[,j][change$taxa == i] <- length(quick$taxa[quick$taxa == i & quick$trt == j])/3
#     }
# }
# 
# rownames(change) <- change$taxa
# change <- change[order(change$taxa),]
# #change <- change[,-1]
# 
```
